...
Portfolio project: R&D Analyst simulation, phân tích recipes_10k.csv (Food.com sample). Toàn bộ số liệu đã qua audit độc lập, verify trực tiếp trên dữ liệu ở từng bước. Ghép từ 3 phần (Part 1/2/3) đã audit riêng lẻ trước đó.

F&B R&D ANALYTICS AUDIT REPORT
PART 1/3 — Phase 0 (Data Verification) → Phase 2 (Measurement & Variable Design)

Vai trò: R&D Analyst (portfolio simulation) Dataset: recipes_10k.csv — 10,000 dòng, sample từ Food.com Recipes and Reviews dataset (Kaggle) Phương pháp luận: F&B Analytical Question & Methodology Audit — quy trình đa giai đoạn với freeze gate bắt buộc giữa các phase, mỗi con số/quyết định phải verify trực tiếp trên dữ liệu trước khi khóa.
...
≥3 làm primary (N=2,197, giữ 19 category N≥30). ≥5 và ≥10 làm robustness check bắt buộc (Part 7).
...
19-category list đầy đủ (N sau lọc, sắp xếp giảm dần): Dessert(206), Lunch/Snacks(146), One Dish Meal(136), Vegetable(129), Breakfast(90), Chicken(80), Beverages(75), Chicken Breast(70), Potato(68), Pork(56), Sauces(55), Breads(50), Quick Breads(47), Meat(44), Pie(40), Drop Cookies(38), Bar Cookie(38), < 60 Mins(31), Yeast Breads(30).
...
Trạng thái: Phase 0-2 đã qua audit độc lập, verify trực tiếp trên recipes_10k.csv cho từng số liệu. Sẵn sàng chuyển Part 2 — Phase 3 (Competing Variables) đến Phase 5 (Statistical Method Selection).

F&B R&D ANALYTICS AUDIT REPORT
PART 2/3 — Phase 3 (Competing Variables) → Phase 5 (Statistical Method Selection)
...
Không có potential confounder nào bị bỏ qua mà không có xử lý/justification — đáp ứng yêu cầu bắt buộc của Part 4 methodology.
...
Trạng thái: Phase 3-5 đã qua audit độc lập, verify trực tiếp trên recipes_10k.csv. 1 correction thật đã xảy ra trong phạm vi này: nhầm tên category < 30 Mins thay vì < 60 Mins trong bảng rating-missingness ở Phase 3 (đã sửa, phản ánh đúng trong bảng 3.3 ở trên). Sẵn sàng chuyển Part 3 — Phase 6 (kết quả thực nghiệm) đến Final Memo.

F&B R&D ANALYTICS AUDIT REPORT
PART 3/3 — Phase 6 (Kết quả thực nghiệm) → Phase 9 (Opportunity Screening) + Final Memo
...
Cutpoint tertile: ≤10 phút / 10–15 phút / >15 phút (tính trên N=1,429, tái lập full-precision, ổn định qua 2 lần chạy độc lập).

Fisher combined χ²=29.405, df=34, p=0.692, weighted ε²≈-0.004 (≈0) → Không có ý nghĩa thống kê.

Ghi chú minh bạch: một lần reproduce độc lập khác cho χ²≈28.9, p≈0.716 (cùng k=17, df=34) — chênh lệch nhỏ, nhiều khả năng do khác biệt tie-handling giữa 2 lần triển khai độc lập, không phải lỗi N/data-handling. Cả hai giá trị đều cách xa ngưỡng 0.05, không ảnh hưởng kết luận — đây là chênh lệch không đáng kể, không phải correction.
...
Ceiling effect ở outcome: skew=-2.52; 71.1% recipe Rating=5.0 tuyệt đối, 97.5% ≥4.0.
Selection bias: 48.3% recipe (4,827/10,000) không có AggregatedRating — không phải random missing.
N-shrinkage qua các tầng lọc validity: category-effect 1,429/10,000 (14.3%), prep-time-effect 1,323/10,000 (13.2%), nutrition-effect 898/10,000 (8.98%) — ba con số tách riêng, không gộp chung.
...
Trạng thái: Phase 6-Final Memo đã qua audit độc lập. 2 correction thật trong phạm vi này:

Phase 6: N=907 (pool trước lọc category) báo cáo nhầm thành N test thực tế (đúng là 898).
Final Memo: N=1,429 (của category-effect) gộp nhầm dùng chung cho prep-time-effect (đúng là 1,323).

1 chênh lệch không đáng kể (không phải correction) ghi nhận ở Phase 6.2: 2 lần reproduce độc lập Fisher-combine cho χ²=29.405 vs χ²≈28.9 (cùng k=17, df=34, cùng p≈0.69-0.72) — không đổi kết luận, không tính là lỗi.

Project hoàn tất — Part 1/2/3.

PHỤ LỤC — REPRODUCIBILITY NOTES

Các chi tiết kỹ thuật cần đúng để tái lập kết quả trên Colab hoặc môi trường khác. Mọi điểm dưới đây đã verify trực tiếp trên recipes_10k.csv trong quá trình audit.
...
Lưu ý đã verify: case PT0S (chỉ có giây, không giờ/phút) — có 318 recipe dạng này trong dataset — parse đúng thành 0 phút với hàm trên (vì t khởi tạo mặc định = 0, không phụ thuộc regex H/M match hay không). Đây không phải điểm gây missing value; đã verify PrepTimeMin có 0 giá trị missing trong subset 19-category (N=1,429). Hàm trên không xử lý phần giây (bỏ qua S) — nếu cần độ chính xác đến giây thì cần bổ sung re.search(r'(\d+)S', s), nhưng với mục đích phân tích ở phút thì không ảnh hưởng.
